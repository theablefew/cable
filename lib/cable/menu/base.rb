class Cable::Menu::Base < ActiveRecord::Base
    ...
    def generate_marketable_url
      ...
      unless self.external_link? || self.direct_link?
        self.marketable_url = ((self.ancestors.sort_by{|anc| anc.level}.collect{ |a| a.title_or_url.parameterize.to_s } << self.title_or_url.parameterize.to_s))[1..-1].join("/")
