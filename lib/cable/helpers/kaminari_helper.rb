module Cable
  ...
  #this method will be included in cable 0.9.11 and can be removed once upgraded
  # def single_column?
  ...
  #   
      ...
      # :nodoc:
      def render_association(association, f, new_object, path)
        f.fields_for(association, new_object, :child_index => "new_#{association}") do |builder|
          render(path + association.to_s.singularize + "_fields", :f => builder, :dynamic => true)
        end
      end
      ...
      def link_to_add_association(*args, &block)
        if block_given?
              ...
              html_options[:'data-template'] = CGI.escapeHTML(render_association(association, f, new_object, path)).html_safe
